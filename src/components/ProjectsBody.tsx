...
import { CaretRightIcon, CaretUpIcon, GlobeIcon } from "./icons/RetroIcons";
...
const ProjectsBody:React.FC<ProjectsBodyProps> = ({ list, selected, onSelect }) => {
  ...
  return (
    <div className="flex h-full w-full flex-col text-[10px] sm:text-[11px] bg-[#f3f3f3]">
      {/* Toolbar / Path */}
      <div className="flex items-center gap-2 sm:gap-3 border-b border-black bg-[#e4e4e4] px-2 py-1">
        <span className="font-bold">Projects:</span>
        <span className="px-1 border border-black bg-white hidden xs:inline">Macintosh HD</span>
        <span className="text-black/50">/</span>
        <span className="px-1 border border-black bg-white hidden xs:inline">Portfolio</span>
        <span className="text-black/50">/</span>
        <span className="px-1 border border-black bg-white">Web</span>
        <div className="ml-auto flex items-center gap-1">
          <button onClick={()=>setView('list')} className={`border border-black px-2 py-0.5 text-[10px] sm:text-[11px] ${view==='list'?'bg-black text-white':'bg-white hover:bg-black hover:text-white'}`}>List</button>
          <button onClick={()=>setView('icons')} className={`border border-black px-2 py-0.5 text-[10px] sm:text-[11px] ${view==='icons'?'bg-black text-white':'bg-white hover:bg-black hover:text-white'}`}>Icons</button>
        </div>
      </div>

      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar (categorías ficticias) */}
        <aside className="hidden md:flex w-40 shrink-0 border-r border-black bg-[#efefef] p-2 flex-col gap-1 overflow-auto">
        ...
        <div className="flex flex-1 flex-col border-r border-black bg-white overflow-hidden">
          ...
          {view==='list' && (
            <div className="grid grid-cols-[auto_1fr_auto_auto] gap-2 border-b border-black bg-[#e9e9e9] px-2 py-1 font-semibold select-none text-[9px] sm:text-[10px]">
          ...
          <div className="relative flex-1 overflow-auto">
                ...
                {sorted.map((p,i)=>{
                  ...
                  return (
                    <li key={p.id}
                      ...
                      <span className="text-[9px] sm:text-[10px] max-w-[100px] sm:max-w-[120px] truncate">{p.tech.join(', ')}</span>
                      {p.url? <a onClick={e=>e.stopPropagation()} href={p.url} target="_blank" rel="noreferrer" className="underline text-[9px] sm:text-[10px] hover:no-underline">Open</a>: <span className="text-black/40 text-[9px] sm:text-[10px]">—</span>}
                    </li>
                  );
                })}
              </ul>
            ...
            {view==='icons' && (
              <div className="p-2 sm:p-3 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 sm:gap-4">
                {sorted.map((p,i)=> {
                  const sel = selectedIds.includes(p.id);
                  return (
                    <button key={p.id}
        ...
        <div className="hidden lg:flex w-[300px] shrink-0 flex-col bg-[#fafafa]">
          ...
          <div className="p-3 flex flex-col gap-3 overflow-auto text-[11px]">
            <div className="mx-auto border border-black bg-white p-1 shadow-[4px_4px_0_#0003]">
              <img src={active.image} alt={active.title} className="h-24 w-24 sm:h-28 sm:w-28 object-cover" style={{ imageRendering: 'pixelated' as React.CSSProperties['imageRendering'] }} />
            </div>
            <div>
              <p className="font-bold text-[11px] sm:text-[12px] mb-1">{active.title}</p>
              <p className="text-[10px] sm:text-[11px] leading-snug mb-2">{active.short}</p>
              <p className="text-[9px] sm:text-[10px]">Tech: <span className="font-mono">{active.tech.join(', ')}</span></p>
            </div>
