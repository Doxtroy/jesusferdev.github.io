...
const AboutBody: React.FC<AboutBodyProps> = ({
	...
	return (
		<div className="h-full w-full overflow-auto p-4 text-[12px] leading-snug">
			{/* Header / Title */}
			<div className="flex flex-col items-center text-center">
				<img src={pcImage} alt="PC" className="h-24 w-auto object-contain mb-2" />
				<h2 className="text-[20px] font-bold tracking-tight">{brand === 'Me' ? 'Rafindows 95' : brand}</h2>
				<p className="mt-0.5 text-[12px] italic">{version}</p>
			</div>

			{/* Memory Bar */}
			<div className="relative mt-5 mb-8">
				<div className="flex h-8 w-[90%] mx-auto rounded border-[3px] border-black overflow-hidden relative bg-white">
					<div className="disk1 flex items-center justify-end gap-1 pr-2 text-[11px] font-semibold tracking-tight" style={{width:'70%',borderRight:'3px solid black', background:'rgba(0,0,0,0.20)'}}>
						<span>Nudes</span>
					</div>
					<div className="disk2 flex items-center justify-end gap-1 pr-2 text-[11px] font-semibold tracking-tight" style={{width:'20%',borderRight:'3px solid black', background:'rgba(0,0,0,0.10)'}}>
						<span>Life</span>
					</div>
					<div className="disk3 flex items-center justify-end gap-1 pr-2 text-[11px] font-semibold tracking-tight" style={{width:'10%', background:'rgba(0,0,0,0.0)'}}>
						<span>Shit</span>
					</div>
					{/* Labels above */}
					<span className="absolute -top-5 left-0 ml-[5%] font-semibold text-[12px]">{memoryLabelLeft}</span>
					<span className="absolute -top-5 right-0 mr-[5%] font-semibold text-[12px]">{memoryLabelRight}</span>
				</div>
			</div>
			...
			<div className="mt-5 flex justify-center">
				<a href={cvUrl} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-2 border border-black bg-white px-3 py-1 text-[11px] font-semibold shadow-[2px_2px_0_#000] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none">
					<span className="block h-3 w-3 bg-black text-white text-[9px] leading-3 grid place-items-center font-bold">S</span>
